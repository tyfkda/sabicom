...
fn load_state(nes: &mut Nes, rom_path: &Path, slot: u32) -> Result<()> {
...
fn save_state(nes: &Nes, rom_path: &Path, slot: u32) -> Result<()> {
    let state_path = state_filename(rom_path, slot)?;
    std::fs::write(state_path, nes.save_state())?;
    Ok(())
}
...
fn run(rom_path: &Path) -> Result<()> {
    ...
    let mut nes = Nes::try_from_file(&bytes, backup.as_deref(), &Default::default())?;

    let key_config = Nes::default_key_config();
    ...
    while process_events(&mut event_pump) {
        ...
        if im.hotkey(HotKey::SaveState).just_pressed(&im) {
            if let Err(err) = save_state(&nes, &rom_path, cur_slot) {
        ...
        if im.hotkey(HotKey::LoadState).just_pressed(&im) {
            if let Err(err) = load_state(&mut nes, &rom_path, cur_slot) {
        ...
        nes.set_input(&key_input);
        nes.exec_frame(true);

        let frame_buf = nes.frame_buffer();
        ...
        let audio_buf = nes.audio_buffer();
...
impl InputManager {
    fn new(_controllers: Vec<GameController>) -> Self {
